...
data_train = pd.read_pickle("./data_pandas/data_trajectories_train.pkl")

delta_train_min = 0
delta_train_max = 10

cond_delta = (data_train['delta']>=delta_train_min) & (data_train['delta']<=delta_train_max)

data = data_train[cond_delta]


data = data.sample(N_samples_train)
...
for row in range(X_.shape[0]):
    valid_row = np.any(~np.isnan(X_[row]))
    if valid_row:
        tmp = apply_transformation(X_[row], on_off_transformation)
        X_train.append(tmp)
        y_train.append(y_[row])

X_train = np.array(X_train)
y_train = np.array(y_train)

# Validation set
data = pd.read_pickle("./data_pandas/data_trajectories_val.pkl")
 

delta_val_min = delta_train_min
delta_val_max = delta_train_max

cond_delta = (data['delta']>delta_val_min) & (data['delta']<delta_val_max)

data = data[cond_delta]
N_samples_val = 2000
data = data.sample(N_samples_val)
data.reset_index(drop=False, inplace=True)
...
# # Validation set
data = pd.read_pickle("./data_pandas/data_trajectories_test.pkl")
 

delta_test_min = 0
delta_test_max = 10

cond_delta = (data['delta']>delta_test_min) & (data['delta']<delta_test_max)

data = data[cond_delta]
N_samples_test = 2000
data = data.sample(N_samples_test)
data.reset_index(drop=False, inplace=True)
...
X_train = pt.tensor(X_train, dtype=pt.float32).unsqueeze(1)  # [B, C, L]
X_val = pt.tensor(X_val, dtype=pt.float32).unsqueeze(1)  # [B, C, L]
X_test = pt.tensor(X_test, dtype=pt.float32).unsqueeze(1)      # [B, C, L]
y_train = pt.tensor(y_train, dtype=pt.float32)
y_val = pt.tensor(y_val, dtype=pt.float32)
...
set_0 = {
            ...
            "decoder_channels"              :   [32, 32, 64, 64],
            "kernel_size"                   :   3,
            "latent_regressor_hidden_dims"  :   [128],
            "latent_dim"                    :   X_train.shape[2],
            "latent_regressor_architecture" :   "Transformer",
